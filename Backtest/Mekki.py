...
print("incertitude = " + str(incertitude))
logging.info("incertitude = " + str(incertitude))
minimum_price=HAdf.Low[-2]
while(HAdf.Low[-2] < price_entry) :
    print('DD calculation ')
    #get data readi it
    df = getdata_min_ago(symbol, interval=interval, lookback=str(10 * 60))  # global HAdf,RSI_stoch_k,RSI_stoch_d,RSI
    HAdf = HA_transformation(df)
    minimum_price=min(minimum_price,HAdf.Low[-2])

    DD_max = abs((price_entry-minimum_price)/price_entry)
    print('DD max howa ',DD_max)
    time.sleep(3)
    if (HAdf.Open[-1]>=price_entry) :
        print('ryh')
        if position == 'long':
            TP = max(calculated_price_entry + tp / 100 * calculated_price_entry,)
            SL = bf[-1] - incertitude
        else:
            TP = calculated_price_entry - tp / 100 * calculated_price_entry
            SL = tf[-1] + incertitude
